...
function MeetModal({ isOpen, onClose }) {
  ...
  const handleSubmit = async () => {
    ...
    setLoad(true);
    try {
      await axios.post("/api/meetgreet", form);
    } catch {
      /* optimistic */
    }
    setDone(true);
    ...
    setTimeout(() => {
      setDone(false);
      onClose();
      setForm({ name: "", phone: "", email: "", preferredTime: "", notes: "" });
    }, 2200);
  };
        ...
        {done ? (
          <div className="p-10 text-center">
            ...
            <p className="text-sm text-gray-500 mt-1">
              We'll be in touch to arrange a time that suits everyone.
            </p>
          </div>
        ) : (
          <div className="p-6 space-y-4">
            <p className="text-sm text-gray-500">
              Fill in your details and we'll reach out to arrange a convenient
              time.
            </p>
            ...
            <div>
              ...
              <input
                className="field-input"
                placeholder="e.g. Weekday afternoons, or weekend mornings"
            ...
            <div>
              ...
              <textarea
                ...
                rows={2}
                placeholder="Pets, your address, any questions…"
                value={form.notes}
...
export default function MeetGreetSection() {
  ...
  return (
    <section
      ...
      aria-labelledby="meetgreet-heading"
    >
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="max-w-xl mx-auto">
          <div className="bg-white rounded-3xl shadow-card p-8 sm:p-10 text-center">
            <div
              className="w-14 h-14 rounded-full bg-primary-100 flex items-center justify-center mx-auto mb-6"
              aria-hidden="true"
            >
            ...
            <h2
              id="meetgreet-heading"
              className="font-heading font-bold text-gray-900 text-2xl sm:text-3xl mb-3"
            >
              Schedule a Free Meet &amp; Greet
            </h2>
            <p className="text-gray-500 text-sm sm:text-base leading-relaxed mb-7 max-w-md mx-auto">
              We believe in building trust before any booking. Meet us for a
              chat and home visit — no obligation required!
            </p>

            {/* FIXED: checklist properly centered using flex + justify-center */}
            <ul
              className="space-y-3 mb-8 inline-block text-left"
              aria-label="What we'll do at the meet & greet"
            >
              {PERKS.map((perk) => (
                <li
                  key={perk}
                ...
                >
                  <span
                    className="flex-shrink-0 w-5 h-5 rounded-full border-2 border-primary-500 flex items-center justify-center"
                    aria-hidden="true"
                  >
                    <svg
