...
function Toggle({ checked, onChange, label, sub, id }) {
  return (
    <div className="flex items-center justify-between py-3 border-b border-gray-100 last:border-0">
      <div className="pr-4">
        <p className="text-sm font-medium text-gray-800" id={id}>
          {label}
        </p>
  ...
  return (
    <div>
      ...
      <FieldWrap
        ...
        hint="We'll use this to confirm your booking quickly."
      >
        <TextInput
          type="tel"
  ...
  return (
    <div>
      ...
      <FieldWrap
        ...
        hint="Alarm settings, general preferences, anything we should know upfront."
      >
        <TextArea
  ...
  return (
    <div className="bg-gray-50 rounded-xl p-4 border border-gray-100 space-y-3">
      ...
      <div>
        ...
        <input
          className="field-input text-sm"
          placeholder="e.g. 1 cup dry food at 7am and 6pm"
          value={pet.feeding}
          onChange={f("feeding")}
        />
      </div>
      <div>
        ...
        <input
          className="field-input text-sm"
          placeholder="None — or describe (e.g. 1 tablet with evening meal)"
          value={pet.medication}
          onChange={f("medication")}
        />
      </div>
      <div>
        ...
        <input
          className="field-input text-sm"
          placeholder="e.g. Friendly, loves cuddles, scared of loud noises"
          value={pet.personality}
          onChange={f("personality")}
        />
      </div>
      <div>
        ...
        <textarea
          ...
          rows={2}
          placeholder="Sleeping spot, walk schedule, favourite toys…"
          value={pet.notes}
          onChange={f("notes")}
        />
      </div>
  ...
  return (
    <div>
      ...
      <p className="text-sm text-gray-500 mb-5">
        We love caring for animals! If you have pets, tell us a bit about them.
      </p>
...
function Step4({ data, set }) {
  return (
    <div>
      <h3 className="font-heading font-bold text-gray-900 text-lg mb-1">
        Home Access & Preferences
      </h3>
      <p className="text-sm text-gray-500 mb-6">
        Help us care for your home exactly the way you want.
      </p>
      ...
      <div className="mb-5">
        ...
        <FieldWrap
          ...
          hint="Gate codes, alarm instructions, where keys will be left, etc."
        >
          <TextArea
        ...
        <FieldWrap
          label="Keys & Remotes Being Handed Over"
          hint="List each item on a new line."
        >
          <TextArea
      ...
      <div className="mb-5 bg-gray-50 rounded-xl border border-gray-100 p-4 space-y-3">
        ...
        <FieldWrap label="Security Company">
          <TextInput
            ...
            placeholder="e.g. Red Alert, Optimum"
          />
        </FieldWrap>
      ...
      <div className="mb-5">
        ...
        <div className="bg-gray-50 rounded-xl border border-gray-100 px-4 divide-y divide-gray-100">
          <Toggle
            ...
            label="🌿 Plants need watering?"
            sub="We'll follow your instructions"
          />
          <Toggle
            id="pool-toggle"
            checked={data.hasPool}
            onChange={(v) => set("hasPool", v)}
            label="🏊 Pool maintenance required?"
            sub="Let us know the routine"
          />
        </div>
        <div className="mt-3">
          <FieldWrap
            label="🗑️ Rubbish / Bin Day"
            hint="e.g. Wednesdays — black bin to the pavement by 7am"
          >
            <TextInput
              value={data.rubbishDay}
              onChange={(e) => set("rubbishDay", e.target.value)}
              placeholder="Day and any special instructions"
            />
          </FieldWrap>
        </div>
      </div>
      ...
      <div>
        ...
        <div className="bg-gray-50 rounded-xl border border-gray-100 px-4 divide-y divide-gray-100">
          <Toggle
            ...
            label="📶 WiFi access for us?"
            sub="We're both online students — much appreciated!"
          />
          <Toggle
            id="updates-toggle"
            ...
            sub="We'll send an update each evening so you can relax"
          />
        </div>
        {data.wifi && (
          <div className="mt-3">
    ...
    <div>
      ...
      <div className="rounded-xl border border-gray-100 overflow-hidden mb-5">
        {rows.map(([k, v], i) => (
          <div
            key={k}
            className={`flex justify-between text-sm px-4 py-2.5 ${i % 2 === 0 ? "bg-gray-50" : "bg-white"}`}
          >
            <span className="text-gray-500 font-medium min-w-[110px]">{k}</span>
            <span className="text-gray-900 text-right ml-4 break-words max-w-[55%]">
              {v || "—"}
            </span>
          </div>
        ))}
      ...
      <div className="bg-primary-50 border border-primary-100 rounded-xl p-4 text-xs text-primary-800 leading-relaxed mb-4">
        By submitting this request you confirm that the above information is
        accurate. We'll review it and respond within 24 hours via email or
        WhatsApp.
      </div>

      {error && (
        <div
          className="bg-red-50 border border-red-200 text-red-700 text-sm rounded-xl p-4 flex items-start gap-2"
          role="alert"
        >
          <svg
...
export default function BookingModal({ isOpen, onClose, initialDates = {} }) {
  ...
  // IMPORTANT: Must be BEFORE any early returns to obey Rules of Hooks
  const set = useCallback(
    (key, val) => setData((d) => ({ ...d, [key]: val })),
    [],
  );
      ...
      >
        <div className="bg-white rounded-3xl p-8 sm:p-10 max-w-sm w-full text-center shadow-xl">
          ...
          <p className="text-gray-500 text-sm mb-6">
            Thank you! We'll review your details and confirm within 24 hours via
            email or WhatsApp.
          </p>
