...
const bookingConfirmGuest = (b) => base(
  ...
  <table width="100%" cellpadding="0" cellspacing="0">
    ...
    ${row('Check-out', b.dateTo)}
    ${row('Address',   b.address)}
    ${row('Pets',      b.hasPets ? (b.pets.filter(p=>p.name).map(p=>p.name).join(', ') || 'Yes') : 'None')}
  </table>
  ...
  <p style="font-size:13px;color:${MUTED};">See you soon!<br/><strong style="color:${DARK};">Layla &amp; [Your Name]</strong><br/>Gonubie House Sitting</p>
  `
)

// ── 3. BOOKING CONFIRMED — sent to guest when owners accept ──────────────
const bookingAcceptedGuest = (b) => base(
  ...
  <p style="font-size:13px;color:${MUTED};">Warm regards,<br/><strong style="color:${DARK};">Layla &amp; [Your Name]</strong></p>
  `
)

// ── 4. BOOKING CANCELLED ─────────────────────────────────────────────────
const bookingCancelledGuest = (b) => base(
